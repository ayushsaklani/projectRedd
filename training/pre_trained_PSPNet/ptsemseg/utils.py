...
class residualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.quant_add.add(out,residual)
        out = self.relu(out)
        return out
...
class FRRU(nn.Module):
    ...
    def forward(self, y, z):
        ...
        z_prime = self.quant_add.add(z , x)
...
class RU(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.conv2(x)
        return self.quant_add.add(x,incoming)
...
class residualConvUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.quant_add.add( x,input)
...
class multiResolutionFusion(nn.Module):
    ...
    def forward(self, x_high, x_low):
        ...
        return self.quant_add.add(low_upsampled ,high_upsampled)
        ...
        if self.fusion_mode == "cat":  # pspnet: concat (including x)
            ...
            for i, (module, pool_size) in enumerate(
                ...
                if self.model_name != "icnet":
                    out = module(out)
                out = F.upsample(out, size=(h, w), mode="bilinear")
                pp_sum = self.quant_add.add(pp_sum , out)
...
class bottleNeckPSP(nn.Module):
    ...
    def forward(self, x):
        ...
        residual = self.cb4(x)
        return F.relu(self.quant_add.add(conv , residual), inplace=True)
...
class bottleNeckIdentifyPSP(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.relu(self.quant_add.add(x,residual), inplace=True)
...
class cascadeFeatureFusion(nn.Module):
    ...
    def forward(self, x_low, x_high):
        ...
        high_fused_fm = F.relu(self.quant_add.add(low_fm , high_fm), inplace=True)
